...
async fn initialiser<M>(middleware: &'static M) -> Result<(&'static DocumentsDomainManager, FuturesUnordered<JoinHandle<()>>), CommonError>
where M: Middleware + IsConfigNoeud
{
    ...
    let gestionnaire = DocumentsDomainManager::new(instance_id);
...
async fn thread_entretien<M>(_gestionnaire: &DocumentsDomainManager, middleware: &M)
where M: Middleware
{
